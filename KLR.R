...
{
  if (klogreg$kernel == 'M_AllKendall') {
    K = newData %*% klogreg$alpha
  } else if (klogreg$kernel == "kendall_total") {
...
cv_klr = function(data, params, fold=5, verbose=F) {
  ...
  for (ind in 1:nParams) {
    ...
    for (f in 1:fold) {
      ...
      if (p$kernel == "M_AllKendall") {
        curTestK = M_AllKendall(bandwidth = p$bandwidth, curTest$x, curTrain$x)
        curpred = Klogreg_predict(curmodel, curTestK)
      } else if (p$kernel == "kendall_total") {
        curTestK = curTest
        curpred = Klogreg_predict(curmodel, curTest)
      }
