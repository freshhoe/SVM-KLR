# Gene expression data simulation function ----

GeneSimulate = function (datasets, rep, prop, kernel, bandwidth, C, lambda, tol, maxiter, rank = T){
  Start = paste("Starting the", rep, "times iteration :", date(), "\n")
  cat(Start)
  call <- match.call()
  
  if(datasets == 'petricoin'){ 
    data = eval(parse(text = load('ocpbsii.RData')))
  } else {
    string = c('alon', 'gordon', 'pomeroy', 'singh')
    data = eval(parse(text = data(list=string[which(string %in% datasets)])))
    if(datasets == 'singh'){ data$x = as.matrix(data$x) }
  }
